...
class AuthController 
{
    ...
    public function registro(Request $request)
    {
        ...
        try {
            ...
            // Si el servidor de APIMarket falla (500, 502, etc.)
            if ($respuestaSEP->serverError()) {
                return response()->json([
                    'mensaje' => 'No fue posible verificar la cédula en este momento. Intenta más tarde.',
                ], 503);
            }

            $datosSEP = $respuestaSEP->json();

            // Cédula no encontrada en el padrón
            if (
                empty($datosSEP['success']) ||
                $datosSEP['success'] !== true  ||
                empty($datosSEP['data'])
            ) {
                return response()->json([
                    'errors'  => ['cedula' => ['La cédula no está registrada en el padrón del SEP.']],
                    'mensaje' => 'La cédula profesional no es válida o no existe en el Registro Nacional de Profesionistas.',
                ], 422);
            }

            // Construir nombre completo desde los datos del SEP
            $registro   = $datosSEP['data'][0];
            $nombreSEP  = trim(
                ($registro['nombre']  ?? '') . ' ' .
                ($registro['paterno'] ?? '') . ' ' .
                ($registro['materno'] ?? '')
            ...
            return response()->json([
                'mensaje' => 'No se pudo conectar con el servicio de verificación de cédulas. Intenta más tarde.',
            ], 503);
        }

        // 3. Lógica para "Otra" especialidad
        ...
        // 4. Crear el Doctor — el nombre viene del SEP para mayor confianza
        $doctor = \App\Models\Doctor::create([
            'nombre'          => $nombreSEP ?: $request->nombre,
        ...
        return response()->json([
            ...
            'nombre_sep' => $nombreSEP, // El frontend puede mostrarlo como confirmación
        ], 201);
    }
